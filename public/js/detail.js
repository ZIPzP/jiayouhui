/* 目的地详情弹窗（共用模块）
 * 首页/目的地/平台热度等页面共用。
 * 用法：页面需包含 #detailModal 与 #lightbox 结构，并 <script src="/js/detail.js?v=..."></script>，
 *       然后调用 window.openDestDetail(id) 打开详情。
 * 特性：打开时 pushState、返回键/✕ 关闭后精确恢复到打开前滚动位置。
 */
(() => {
  'use strict';
  const state = { modalOpen: false, scrollY: null };
  ...
  async function openDetail(id) {
    ...
    if (body) body.innerHTML = '<p style="padding:40px;text-align:center">加载中…</p>';
    try {
      const { destination: d } = await app.api('/api/destinations/' + encodeURIComponent(id));
      renderDetail(d);
    } catch (e) {
      if (body) body.innerHTML = `<p style="padding:40px;text-align:center;color:var(--danger)">加载失败：${app.esc(e.message)}</p>`;
    }
  }
  function renderDetail(d) {
    ...
    if (hero && d.cover) { const probe = new Image(); probe.onload = () => app.setBg(hero, d.cover); probe.src = d.cover; }
  ...
  function openLightbox(i) {
    ...
    if (img) img.src = lb.urls[lb.index];
    const cap = document.getElementById('lightboxCaption');
    if (cap) cap.textContent = lb.titles[lb.index] || '';
    box.hidden = false;
  ...
  window.openDestDetail = openDetail;
})();