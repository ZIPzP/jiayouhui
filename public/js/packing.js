(() => {
  ...
  window.pageInit = async function () {
    ...
    document.getElementById('pf-dest').innerHTML = opts;
    bindEvents();
    ...
    if (quick) {
      ...
      try {
        ...
        if (v.destinationId) document.getElementById('pf-dest').value = v.destinationId;
  ...
  function formValues() {
    return {
      destinationId: document.getElementById('pf-dest').value,
  ...
  function renderResult(el, data, vals) {
    ...
    el.innerHTML = `
      <div class="result-head">
        <h3>🎒 ${app.esc(dest.name || '')} · ${app.esc(data.monthLabel || '')}出行清单</h3>
      ...
      <div class="result-actions">
        ...
        <button class="btn btn-ghost read-aloud" type="button" data-read="${app.esc(dest.name + '出行清单。' + (data.items || []).map(i => i.name + '，' + (i.reason || '')).join('。') + '。' + (data.tips || []).join('。'))}">🔊 朗读清单</button>
      </div>`;
