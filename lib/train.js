...
async function queryTrainsWithPrices(fromCity, toCity, date, n = 8) {
  let base = await queryTrains(fromCity, toCity, date, n);
  let usedDate = date;
  let refNote = false;
  // 超出 12306 预售期（约15天）会查询失败：改用近期可查日期，作为参考班次（车次基本每日固定）
  if (!base.ok || !base.trains.length) {
    const fd = new Date();
    fd.setDate(fd.getDate() + 7);
    const fallback = fd.toISOString().slice(0, 10);
    const b2 = await queryTrains(fromCity, toCity, fallback, n);
    if (b2.ok && b2.trains.length) { base = b2; usedDate = fallback; refNote = true; }
  }
  if (!base.ok) return { ok: false, error: base.error, trains: [] };
  const price = await queryPrices(fromCity, toCity, usedDate);
  const priceMap = {};
  if (price.ok) for (const p of price.trains) priceMap[p.no] = p.prices || {};
  const trains = base.trains.map((x) => ({ ...x, prices: priceMap[x.no] || {} }));
  return { ok: true, trains, date: usedDate, refNote };
}
