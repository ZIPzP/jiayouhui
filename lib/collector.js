...
/** 按当月热门重新排名：使用目的地 monthHeat[当月] 作为热度（无则用基础 heat） */
function monthlyRank(items, month) {
  const m = Number(month) || new Date().getMonth() + 1;
  return (items || [])
    .map((it) => {
      const mh = it.monthHeat && it.monthHeat[String(m)];
      const heat = mh != null ? Number(mh) : (Number(it.heat) || 0);
      return { ...it, heat };
    })
    .sort((a, b) => b.heat - a.heat)
    .map((it, i) => ({ ...it, rank: i + 1, month: m, monthLabel: m + '月' }));
}

module.exports = { collect, merge, monthlyRank };