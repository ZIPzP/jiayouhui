  ...
  return `请为以下家庭旅行生成一份“逐日详细行程规划”，输出 JSON（不要输出其他文字）：
{
  ...
  "transportPlan": { "outbound": "去程交通安排（说明方式/大致耗时/是否需中转；不要编造具体车次号与票价）", "inbound": "返程交通安排（同上）", "local": "当地交通建议" },
...
- 交通时效特别提示：涉及飞机时只写「建议城市→城市、大致飞行时长」，不要编造航班号（如MU123）、起飞时刻或精确机票价格；涉及火车时同样只写方式与大致耗时。所有班次与票价一律提醒用户以12306/航司官方实时查询为准。
