...
function rulePlan(params, lang) {
  ...
  if (en) {
    ...
    if (transport === '高铁' && HAINAN.includes(destNorm) && !HAINAN.includes(originNorm)) {
      transportNote = '⚠️ ' + destNorm + ' is on Hainan Island and mainland high-speed rail cannot reach it directly (the train crosses by ferry and takes much longer). Flying is recommended; the island ring railway links the cities.';
    }
  ...
  for (let i = 0; i < n; i++) {
    ...
    const title = isLast ? (en ? 'Heading home' : '返程')
      : (dayNum === 1 ? (en ? 'Arrival & first look' : '抵达·初体验') : (hl ? hl.title : (en ? 'A day in the city' : '市区休闲')));
    const schedule = isLast ? [
      ...
      { time: T('上午'), activity: dayNum === 1 ? (en ? 'Arrive and settle into the hotel' : '抵达目的地，前往酒店安顿') : (hl ? hl.title : (en ? 'City sights' : '市区景点')), detail: dayNum === 1 ? (en ? `${transportNote} Check in and have lunch nearby.` : `${transportNote} 抵达后办理入住，附近午餐。`) : (hl ? hl.text : (en ? 'Free time' : '自由活动')) },
      { time: T('下午'), activity: dayNum === 1 ? (hl2 ? hl2.title : (en ? 'City stroll' : '市区漫步')) : (hl2 ? hl2.title : (en ? 'At leisure' : '休闲')), detail: hl2 ? hl2.text : '' },
  ...
  return {
    ...
    paceNote,
    tips: [
      destination.elderlyFriendly || (en ? 'Balance activity with rest.' : '注意劳逸结合。'),
