...
/* 英文翻译表：打包物品与目的地数据（构建时生成，无运行时依赖） */
function loadI18n(file) { try { return JSON.parse(fs.readFileSync(path.join(DATA_DIR, file), 'utf8')); } catch (e) { return {}; } }
const PACK_I18N = loadI18n('packing-i18n.json');
const DEST_I18N = loadI18n('dest-i18n.json');
const trP = (s) => (PACK_I18N[String(s)] || s);
const trD = (s) => (DEST_I18N[String(s)] || s);
const SEASON_EN = { '春': 'spring', '夏': 'summer', '秋': 'autumn', '冬': 'winter' };
const MONTH_EN = ['January','February','March','April','May','June','July','August','September','October','November','December'];
const SEASON_ADVICE_EN = {
  '春': 'Spring warms up quickly but day-night temperature gaps are wide and it can be misty — layer up and carry an umbrella.',
  '夏': 'Hot and rainy with strong UV — use sun protection, drink plenty of water and pack rain gear for afternoon thunderstorms.',
  '秋': 'Clear skies but wider day-night gaps — add a light jacket in the evening and moisturise in the dry air.',
  '冬': 'Cold, and damper regions feel colder — warm clothing is essential.'
};
const daysEn = (s) => String(s || '').replace(/(\d+)\s*-\s*(\d+)\s*天/, '$1–$2 days').replace(/(\d+)\s*天/, '$1 days');
let destCache = null;
let packCache = null;
...
function buildRuleBased(params, lang) {
  const en = lang === 'en';
  ...
  const add = (it) => {
    ...
    o._zh = it.name; // 保留中文名用于「超详细配套」匹配
    if (en) { o.name = trP(it.name); o.reason = trP(it.reason); o.category = trP(it.category); }
    items.push(o);
  };
  ...
  // 超详细模式：懒人配套 + 补充包
  if (String(mode) === '超详细') {
    const names = items.map((i) => i._zh || i.name);
  ...
  const m = Number(month) || new Date().getMonth() + 1;
  const destLabel = en ? (destination.enName || destination.name) : destination.name;
  const seasonAdvice = en
    ? (SEASON_ADVICE_EN[season] || 'Check the local forecast before you travel.')
    : ({ '春': '气温回升但昼夜温差大，多雨雾，建议“洋葱式”叠穿并随身带伞。',
         '夏': '炎热多雨，紫外线强，注意防晒、补水、防中暑，并备雨具应对午后雷阵雨。',
         '秋': '天高气爽但昼夜温差增大，早晚加薄外套，空气干燥注意保湿。',
         '冬': '寒冷（湿冷地区体感更冷），务必做好保暖，注意防寒防冻。' }[season] || '出行前请查询当地天气预报。');
  const weatherAdvice = en
    ? `${MONTH_EN[m - 1]} in ${destLabel}: ${seasonAdvice}${climate ? ' (Local climate: ' + trD(climate) + ')' : ''}`
    : `${m}月去${destination.name}：${seasonAdvice}${climate ? '（当地气候：' + climate + '）' : ''}`;

  const tips = [
    notes ? (en ? '✍️ Your request: ' + notes : '✍️ 用户特别要求：' + notes) : null,
    en ? `Suggested length: ${daysEn(destination.suggestDays)} — keep the pace slow and leave half a day flexible.` : `建议游玩 ${destination.suggestDays}，行程放慢，每天留半天机动时间。`,
    en ? 'Keep copies of your ID and medicine in both your daypack and your suitcase.' : '证件、药品分别放随身包与行李箱各一份，以防丢失。',
    en ? (trD(destination.elderlyFriendly) || 'Balance activity with rest.') : (destination.elderlyFriendly || '注意劳逸结合。'),
    Number(durationDays) >= 5 ? (en ? 'It is a long trip — plan one easy day to rest and avoid rushing.' : '行程较长，中途安排一天轻松休整，避免连续奔波。') : null
  ].filter(Boolean);

  items.forEach((i) => { delete i._zh; });
  return {
    provider: 'rule',
    season: en ? SEASON_EN[season] : season,
    monthLabel: en ? MONTH_EN[m - 1] : monthName(month),
    weatherAdvice,
    tips,
    items,
...
function ruleBasedGuide(destination, lang) {
  const en = lang === 'en';
  const destLabel = en ? (destination.enName || destination.name) : destination.name;
  const hl = (destination.highlights || []).map((h) => `· ${en ? trD(h.title) : h.title}：${en ? trD(h.text) : h.text}`);
  if (en) {
    return {
      provider: 'rule',
      title: `${destLabel} · Family travel guide`,
      summary: trD(destination.description),
      sections: [
        { heading: 'Overview', content: `Suggested length: ${daysEn(destination.suggestDays)}. ${trD(destination.description)}` },
        { heading: 'Highlights', content: hl.join('\n') },
        { heading: 'Tips for seniors', content: trD(destination.elderlyFriendly) },
        { heading: 'Before you go', content: trD(destination.packingNote) }
      ],
      tips: ['Book tickets through official channels and avoid peak times', 'Bring ID (and the child’s household register) for elders and children', 'Check the local weather and adjust your plans'],
      generatedAt: new Date().toISOString()
    };
  }
  return {
...
async function recommend(params, aiOverrides) {
  ...
  const fallback = buildRuleBased(params, (aiOverrides || {}).lang);
...
async function aiGuide(destination, aiOverrides) {
  const settings = ai.getSettings(aiOverrides);
  const fallback = ruleBasedGuide(destination, (aiOverrides || {}).lang);
  if (!settings.apiKey) return fallback;
  try {
    const content = await ai.chat(
      settings,
